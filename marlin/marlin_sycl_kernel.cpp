...
#include "marlin_cuda_kernel.dp.hpp"
...
using FragS = Vec<sycl::half2, 1>; // quantization scales
...
inline void cp_async4_pred(sycl::queue &q_ct1, void* smem_ptr, const void* glob_ptr, bool pred = true) {
#if defined(DPCT_COMPATIBILITY_TEMP) && DPCT_COMPATIBILITY_TEMP >= 800
  const int BYTES = 16;
  bool p = pred;
  q_ct1.submit([&](sycl::handler &cgh){
  
    //sycl::accessor<uint32_t, 1, sycl::access::mode::read_write, sycl::target::local> shared_acc((BYTES)/sizeof(uint32_t), cgh);
    cgh.parallel_for(
             sycl::nd_range<3>(sycl::range<3>(1, 1, BYTES / sizeof(uint32_t)), sycl::range<3>(1, 1, BYTES / sizeof(uint32_t))),
            [=](sycl::nd_item<3> item_ct1){
            
           auto global_id = item_ct1.get_global_linear_id();
           auto shared_ptr = static_cast<uint32_t*>(smem_ptr);
           auto global_ptr = static_cast<const uint32_t*>(glob_ptr);
              
           if(p && global_id < BYTES/(sizeof(uint32_t))){
                shared_ptr[global_id] = global_ptr[global_id];
           }
...
inline void cp_async4_stream(void* smem_ptr, const void* glob_ptr) {
#if defined(DPCT_COMPATIBILITY_TEMP) && DPCT_COMPATIBILITY_TEMP >= 800
  const int BYTES = 16;
  //To-do : Use L2 cache eviction for SYCL
  bool p = pred;
  q_ct1.submit([&](sycl::handler &cgh){
...
// Async copy fence.
inline void cp_async_fence(const sycl::nd_item<3> &item_ct1) {
#if defined(DPCT_COMPATIBILITY_TEMP) && DPCT_COMPATIBILITY_TEMP >= 800
...
inline const float* mma(sycl::queue &q,  FragA& a_frag,  FragB& frag_b, FragC& frag_c){
     float data[4];
     for(auto i=0;i<4;i++) data[i]=frag_c[i];
      sycl::buffer<float, 1> frag_c_buffer(data, 4);
     q.submit([&](sycl::handler &cgh) {
         sycl::accessor dacc_write(frag_c_buffer, cgh, sycl::write_only);
                             
        cgh.parallel_for(sycl::nd_range<3>({1, 1, 1}, {1, 1, 1}),
                         [=](sycl::nd_item<3> item_ct1) {
                             for (int i = 0; i < 4; ++i) {
                                  sycl::half2 a1 = a_frag[i];
                                  sycl::half2 b1 = frag_b[0];
                                  sycl::half2 b2 = frag_b[1];
                                  sycl::half val = sycl::half(frag_c[i]);
                                  // Multiply and accumulate
                                  val += sycl::half(a1.x()) * sycl::half(b1.x()) + 
                                               sycl::half(a1.y()) * sycl::half(b2.x());
                                  dacc_write[i] = float(val);
                              }
                         });
    }).wait();
    /*asm volatile(
    ...
    sycl::host_accessor data_accessor(frag_c_buffer, sycl::read_only);
...
inline int lop3(int a, int b, int c) {
  ...
  int res = 0;
    ...
    for (int i = 0; i < 32; ++i) {
        // Extract the i-th bit of each input
        int bit_a = (a >> i) & 1;
        int bit_b = (b >> i) & 1;
        int bit_c = (c >> i) & 1;

        // Construct an index to access the corresponding bit in the LUT
        int index = (bit_a << 2) | (bit_b << 1) | bit_c;

        // Extract the corresponding bit from the LUT
        int bit_res = (lut >> index) & 1;
...
inline FragB dequant(int q) {
  ...
  FragB frag_b;
  frag_b[0] = *reinterpret_cast<sycl::half2*>(&lo) - *reinterpret_cast<const sycl::half2*>(&SUB);
  frag_b[1] = sycl::fma(*reinterpret_cast<sycl::half2*>(&hi), *reinterpret_cast<const sycl::half2*>(&MUL), *reinterpret_cast<const sycl::half2*>(&ADD));
  return frag_b;
}
...
inline void barrier_acquire_kernel(int* lock, int count, const sycl::nd_item<3> &item_ct1) {
#if defined(DPCT_COMPATIBILITY_TEMP) && DPCT_COMPATIBILITY_TEMP >= 800
  if (item_ct1.get_local_id(2) == 0) {
    int state = -1;
    do
      {sycl::atomic_ref<int, sycl::memory_order::acq_rel, sycl::memory_scope::device, sycl::access::address_space::global_space> atomic_lock(*lock);
      state = atomic_lock.load();}
...
inline void barrier_release_kernel(const sycl::nd_item<3> item_ct1,int* lock, bool reset = false) {
#if defined(DPCT_COMPATIBILITY_TEMP) && DPCT_COMPATIBILITY_TEMP >= 800
  ...
  if (item_ct1.get_local_id(2) == 0) {
    ...
    sycl::atomic_ref<int, sycl::memory_order::relaxed, sycl::memory_scope::device, sycl::access::address_space::global_space> (*lock)+=val;
    //atomic_lock(val);
...
inline void barrier_release(sycl::queue &q, int *lock, bool reset = false){
   q.submit([&](sycl::handler &cgh){
     cgh.parallel_for(sycl::nd_range<3>({1, 1, 1}, {1, 1, 1}),
                         [=](sycl::nd_item<3> item_ct1) {
                           barrier_release_kernel(item_ct1, lock);
...
inline void barrier_acquire(sycl::queue &q, int* lock, int count){

    q.submit([&](sycl::handler &cgh){
      cgh.parallel_for(sycl::nd_range<3>({1, 1, 1}, {1, 1, 1}),
                         [=](sycl::nd_item<3> item_ct1) {                   
                       
                       barrier_acquire_kernel(lock, count, item_ct1);
...
void Marlin(
  ...
  const sycl::int4* __restrict__ s, // fp16 quantization scales of shape (k/groupsize)xn
  int  prob_m, // batch dimension m
  int  prob_n, // output dimension n
  int  prob_k, // reduction dimension k
  int* locks ,
  uint8_t *dpct_local// extra global storage for barrier synchronization
) {
  ...
  int iters = ceildiv(k_tiles * n_tiles * parallel, item_ct1.get_group_range(2));
  ...
  int a_gl_rd = a_gl_stride * (item_ct1.get_local_id(2) / a_gl_rd_delta_o) + (item_ct1.get_local_id(2) % a_gl_rd_delta_o);
  a_gl_rd += a_gl_rd_delta_o * slice_row;
  // Shared write index of current thread.
  int a_sh_wr = a_sh_stride * (item_ct1.get_local_id(2) / a_gl_rd_delta_o) + (item_ct1.get_local_id(2) % a_gl_rd_delta_o);
  // Shared read index.
  int a_sh_rd = a_sh_stride * ((item_ct1.get_local_id(2) % 32) % 16) + (item_ct1.get_local_id(2) % 32) / 16;
  a_sh_rd += 2 * ((item_ct1.get_local_id(2) / 32) / (thread_n_blocks / 4));

  int b_gl_rd = b_gl_stride * (item_ct1.get_local_id(2) / b_sh_stride) + (item_ct1.get_local_id(2) % b_sh_stride);
  b_gl_rd += b_sh_stride * slice_col;
  b_gl_rd += b_gl_rd_delta_o * slice_row;
  int b_sh_wr = item_ct1.get_local_id(2);
  int b_sh_rd = item_ct1.get_local_id(2);

  int s_gl_rd = s_gl_stride * ((thread_k_blocks * slice_row) / group_blocks) + s_sh_stride * slice_col + item_ct1.get_local_id(2);
  ...
  sycl::int4* sh_s = sh_b + (stages * b_sh_stage);
  ...
  auto wait_for_stage = [&] () {
    ...
    */
    sycl::ext::oneapi::experimental::this_nd_item<3>().barrier();
  };
  ...
  auto fetch_to_registers = [&] (int k, int pipe) {
    ...
    if (group_blocks != -1) {
      sycl::int4* sh_s_stage = sh_s + s_sh_stage * ((group_blocks / thread_k_blocks) * (pipe / (group_blocks / thread_k_blocks)));
  ...
  auto thread_block_reduce = [&] () {
    ...
    if (red_off >= 1) {
      int red_idx = item_ct1.get_local_id(2) / b_sh_stride;
      constexpr int red_sh_stride = b_sh_stride * 4 * 2;
      constexpr int red_sh_delta = b_sh_stride;
      int red_sh_rd = red_sh_stride * (item_ct1.get_local_id(2) / b_sh_stride) + (item_ct1.get_local_id(2) % b_sh_stride);
        ...
        for (int i = red_off; i > 0; i /= 2) {
          if (i <= red_idx && red_idx < 2 * i) {
            ...
            for (int j = 0; j < 4 * 2; j++) {
              ...
              sh[red_sh_wr] = reinterpret_cast<sycl::int4*>(&frag_c)[4 * 2 * m_block + j];
            }
          }
          /*
  ...
  auto global_reduce = [&] (bool first = false, bool last = false) {
    ...
    if (item_ct1.get_local_id(2) < active_threads) {
      int c_gl_stride = prob_n / 8;
      int c_gl_wr_delta_o = 8 * c_gl_stride;
      int c_gl_wr_delta_i = 4 * (active_threads / 32);
      int c_gl_wr = c_gl_stride * ((item_ct1.get_local_id(2) % 32) / 4) + 4 * (item_ct1.get_local_id(2) / 32) + item_ct1.get_local_id(2) % 4;
      c_gl_wr += (2 * thread_n_blocks) * slice_col;
      constexpr int c_sh_wr_delta = active_threads;
      int c_sh_wr = item_ct1.get_local_id(2);

      int row = (item_ct1.get_local_id(2) % 32) / 4;
      ...
      for (int i = 0; i < thread_m_blocks * 4; i++) {
        if (i < (thread_m_blocks - 1) * 4 || 8 * (i / 2) + row < prob_m) {
          if (!first) {
            sycl::int4 c_red = sh[c_sh_wr + i * c_sh_wr_delta];
            #pragma unroll
            for (int j = 0; j < 2 * 4; j++) {
              reinterpret_cast<float*>(&frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)] += sycl::vec<sycl::half, 1>(reinterpret_cast<sycl::half*>(&c_red)[j]).convert<float, sycl::rounding_mode::automatic>()[0];
            }
          }
          if (!last) {
            sycl::int4 c;
            #pragma unroll
            for (int j = 0; j < 2 * 4; j++) {
              reinterpret_cast<sycl::half*>(&c)[j] = sycl::vec<float, 1>(reinterpret_cast<float*>(&frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)]).convert<sycl::half, sycl::rounding_mode::automatic>()[0];
            }
  ...
  auto write_result = [&] () {
    ...
    int c_gl_wr = c_gl_stride * (item_ct1.get_local_id(2) / (2 * thread_n_blocks)) + (item_ct1.get_local_id(2) % (2 * thread_n_blocks));
    c_gl_wr += (2 * thread_n_blocks) * slice_col;
    int c_sh_wr = (4 * c_sh_stride) * ((item_ct1.get_local_id(2) % 32) / 4) + (item_ct1.get_local_id(2) % 32) % 4;
    c_sh_wr += 32 * (item_ct1.get_local_id(2) / 32);
    int c_sh_rd = c_sh_stride * (item_ct1.get_local_id(2) / (2 * thread_n_blocks)) + (item_ct1.get_local_id(2) % (2 * thread_n_blocks));

    int c_gl_wr_end = c_gl_stride * prob_m;

    // We first reorder in shared memory to guarantee the most efficient final global write patterns
    auto write = [&] (int idx, float c0, float c1, FragS& s) {
      sycl::half2 res = sycl::half2(sycl::vec<float, 1>(c0).convert<sycl::half, sycl::rounding_mode::automatic>()[0], sycl::vec<float, 1>(c1).convert<sycl::half, sycl::rounding_mode::automatic>()[0]);
      if (group_blocks == -1) // for per-column quantization we finally apply the scale here
        res = res * s[0];
      ((sycl::half2*) sh)[idx] = res;
    };
  ...
  while (slice_iters) {
    ...
    if (slice_iters == 0) {
      ...
      init_slice();
      if (slice_iters) {
        a_gl_rd = a_gl_stride * (item_ct1.get_local_id(2) / a_gl_rd_delta_o) + (item_ct1.get_local_id(2) % a_gl_rd_delta_o);
        #pragma unroll
        for (int i = 0; i < b_sh_wr_iters; i++)
...
/*
DPCT1026:20: The call to cudaFuncSetAttribute was removed because SYCL currently does not support corresponding setting.
*/
...
\
    const sycl::int4 * A_ptr_ct0 = A_ptr;\
    const sycl::int4 * B_ptr_ct1 = B_ptr;\
    sycl::int4 * C_ptr_ct2 = C_ptr;\
    const sycl::int4 * s_ptr_ct3 = s_ptr;\
    int prob_m_ct4 = prob_m;\
    int prob_n_ct5 = prob_n;\
    int prob_k_ct6 = prob_k;\
    int * locks_ct7 = locks;\
\
    cgh.parallel_for(\
      sycl::nd_range<3>(sycl::range<3>(1, 1, blocks) * sycl::range<3>(1, 1, THREADS), sycl::range<3>(1, 1, THREADS)), \
      [=](sycl::nd_item<3> item_ct1) {\
        Marlin<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>(A_ptr_ct0, B_ptr_ct1, C_ptr_ct2, s_ptr_ct3, prob_m_ct4, prob_n_ct5, prob_k_ct6, locks_ct7, dpct_local_acc_ct1.get_pointer());\
      });\
  }); \
  }
...
int marlin_cuda(
  ...
  if (sms == -1)
    sms = dpct::dev_mgr::instance().get_device(dev).get_max_compute_units();
  ...
  const sycl::int4* A_ptr = (const sycl::int4*) A;
  const sycl::int4* B_ptr = (const sycl::int4*) B;
  sycl::int4* C_ptr = (sycl::int4*) C;
  const sycl::int4* s_ptr = (const sycl::int4*) s;
